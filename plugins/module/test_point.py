    ...
    def parse_test_points(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for sym_name in tp_symbols:
            ...
            if tp_struct:
                ...
                if raw_data:
                    records = self._parse_raw_test_points(raw_data, size, is_32bit)
                    results.extend(records)
        
        return results
    
    def _parse_raw_test_points(self, data: bytes, size: int, is_32bit: bool) -> List[Dict[str, Any]]:
        records = []
        record_size = 32
        ...
        for i in range(num_records):
            ...
            if name_addr != 0:
                record['name'] = self.dump_reader.read_string(name_addr, 32) if hasattr(self, 'dump_reader') else ''
    ...
    def parse_trace_buffer(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for sym_name in trace_symbols:
            ...
            if trace_struct:
                ...
                if raw_data:
                    records = self._parse_raw_trace_records(raw_data, size, is_32bit)
                    results.extend(records)
        
        return results
    
    def _parse_raw_trace_records(self, data: bytes, size: int, is_32bit: bool) -> List[Dict[str, Any]]:
        records = []
        record_size = 24

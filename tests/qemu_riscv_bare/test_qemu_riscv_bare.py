...
class TestQEMURiscvFirmwareAutoParse(unittest.TestCase):
    """QEMU RISC-V (SiFive E) real-run firmware tests.

    Differences from qemu_m4_bare / qemu_r52_bare / qemu_aarch64_bare:
    - CPU is SiFive E31 (RV32IMAC), 32-bit RISC-V
    - QEMU machine 'sifive_e', not mps2-an386 / mps3-an536 / virt
    - Multi-region memory layout: FLASH at 0x20400000 (code+rodata) + RAM at 0x80000000 (data+bss)
    - sifive_e mask ROM hard-codes a jump to 0x20400000 (NOT to ELF entry)
    - Pointers are 4 bytes; DWARF DW_TAG_pointer_type byte_size == 4
    - KEY VALIDATION: char* points into FLASH region, deref must cross regions
    - Validates multi-region dump loading (flash + RAM concatenated) in core/dump_reader.py
    """

    ELF_PATH = os.path.join(os.path.dirname(__file__), 'firmware', 'output', 'img', 'test_firmware_riscv.elf')
    DUMP_PATH = os.path.join(os.path.dirname(__file__), 'firmware', 'output', 'img', 'test_dump_riscv.bin')
    ...
    def setUp(self):
        if not os.path.exists(self.ELF_PATH) or not os.path.exists(self.DUMP_PATH):
            self.skipTest("RISC-V firmware ELF/dump files not found")

        self.elf_parser = ELFParser(self.ELF_PATH)
        profile_loader = ProfileLoader()
        profile = profile_loader.load_profile('test/qemu_riscv_bare')
        regions = profile_loader.get_memory_regions(profile)
        self.dump_reader = DumpReader(self.DUMP_PATH, regions)

    def test_riscv_elf_exists(self):
        """RISC-V ELF and dump both exist and are non-empty."""
        self.assertGreater(os.path.getsize(self.ELF_PATH), 1000)
        self.assertGreater(os.path.getsize(self.DUMP_PATH), 1000)

    def test_riscv_elf_header(self):
        """RISC-V ELF: class 32, machine 'RISC-V', entry in FLASH, has DWARF."""
        header = self.elf_parser.get_elf_header()
        self.assertEqual(header['class'], 32)
        self.assertEqual(header['machine'], 'RISC-V')
        self.assertGreaterEqual(header['entry'], self.FLASH_START,
                               f"entry {header['entry']:#x} should be in FLASH")
        self.assertLess(header['entry'], self.FLASH_END,
                        f"entry {header['entry']:#x} should be in FLASH")
        self.assertIsNotNone(self.elf_parser.dwarfinfo)

    def test_riscv_bss_variables_in_ram(self):
        """All g_* globals have addresses in RAM (0x80000000-0x80001000)."""
    ...
    def test_riscv_scalar_values(self):
        """Scalars after real RISC-V run match simulate_runtime()."""
    ...
    def test_riscv_assert_info_array_expansion(self):
    ...
    def test_riscv_assert_record_details(self):
        """All assert_record_t fields correct in RISC-V dump (with char* deref across regions)."""
        arr = self.elf_parser.parse_struct_auto('g_assert_infos', self.dump_reader)
    ...
    def test_riscv_test_point_array_expansion(self):
    ...
    def test_riscv_trace_buffer_expansion(self):
    ...
    def test_riscv_char_pointer_deref(self):
        """All char* fields in RISC-V dump auto-deref to Python str across regions."""
        arr = self.elf_parser.parse_struct_auto('g_assert_infos', self.dump_reader)
        tps = self.elf_parser.parse_struct_auto('g_test_points', self.dump_reader)
        ...
        for i in range(arr[0]['count']):
            ...
            self.assertIsInstance(r['file_name'], str,
                                  "file_name must auto-deref to str across regions")
            self.assertIsInstance(r['function_name'], str,
                                  "function_name must auto-deref to str across regions")
            self.assertTrue(r['file_name'].endswith('.c'),
                            f"file_name should end with .c, got {r['file_name']!r}")

        for tp in tps:
            if tp['id'] != 0:
                self.assertIsInstance(tp['name'], str,
                                      "test_point.name must auto-deref to str across regions")
                self.assertGreater(len(tp['name']), 2)

    def test_riscv_auto_parse_matches_manual_read(self):
        """parse_struct_auto matches manual read_uint32 (32-bit) on RISC-V."""
        ...
        file_name_ptr = self.dump_reader.read_uint32(ai0_addr + 8)
        ...
        self.assertEqual(manual_str, 'main.c')
